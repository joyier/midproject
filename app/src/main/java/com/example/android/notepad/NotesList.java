...
public class NotesList extends ListActivity {
    ...
    private RelativeLayout container; // 容器视图
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 获取容器视图
        container = (RelativeLayout) findViewById(R.id.notes_list_container);

        // 加载保存的背景颜色
        SharedPreferences prefs = getSharedPreferences(PREFS_NAME, MODE_PRIVATE);
        int savedColor = prefs.getInt(PREF_BACKGROUND_COLOR, getResources().getColor(android.R.color.white));
        container.setBackgroundColor(savedColor);
        ...
        String[] dataColumns = { NotePad.Notes.COLUMN_NAME_TITLE ,NotePad.Notes.COLUMN_NAME_CREATE_DATE,NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE} ;
        ...
        SimpleCursorAdapter adapter
            ...
            public void bindView(View view, Context context, Cursor cursor) {
                ...
                long timestamp = cursor.getLong(cursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE));
                    if (timestamp<=0){
                        timestamp = cursor.getLong(cursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_CREATE_DATE));
                    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        MenuItem searchItem = menu.findItem(R.id.menu_search);  // Ensure this item exists in your menu XML
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextChange(String newText) {
                // Optionally perform live search as the user types
    ...
    private void performSearch(String query) {
        // Build the selection (filter) criteria based on the query
        String selection = NotePad.Notes.COLUMN_NAME_TITLE + " LIKE ? OR " +
                NotePad.Notes.COLUMN_NAME_NOTE + " LIKE ?";
        String[] selectionArgs = new String[]{"%" + query + "%", "%" + query + "%"};

        // Query the content provider to retrieve matching notes
        Cursor cursor = getContentResolver().query(
                NotePad.Notes.CONTENT_URI,  // The URI of your notes content provider
                null,                       // Return all columns
                selection,                  // Selection (filter)
                selectionArgs,              // Selection arguments
                NotePad.Notes.DEFAULT_SORT_ORDER  // Default sort order
        );

        // Update the adapter with the new filtered cursor
        if (cursor != null) {
            SimpleCursorAdapter adapter = (SimpleCursorAdapter) getListAdapter();
            adapter.changeCursor(cursor);  // Update the list with filtered results
        }
    }
        ...
        case R.id.menu_paste:
          ...
          return true;
            case R.id.menu_change_background:
                showColorPickerDialog(); // 显示颜色选择对话框
                return true;
        default:
    ...
    private void showColorPickerDialog() {
        ...
        // 显示颜色选项
        builder.setItems(new CharSequence[]{"white", "Green", "Orange", "black", "grey"}, (dialog, which) -> {
            changeBackgroundColor(colors[which]); // 更新背景颜色
        });
